...
if __name__ == '__main__':
    ...
    try:
        ...
        lan = None
        public_fe = os.environ.get('APP_URL', f'http://localhost:{port}')
        tunnel = None

    print(f'KaamSetu Web App:     http://127.0.0.1:{port}')
    if tunnel:
        print(f'  WiFi + mobile data: {tunnel}')
    if lan:
        print(f'  Same WiFi only:     http://{lan}:{port}')
    print(f'  Email links:        {public_fe}')
